...
| 参数 | 类型 | 必填 | 说明 |
|---|---|---|---|
| `sheet` | string | ❌ | 工作表名称（忽略大小写）或索引（0-based），不传返回全部 |

**请求示例**

```bash
# 获取全部工作表
curl -s https://your-domain.com/api/open/v1/documents/42/sheet-data \
  -H "X-API-Key: ak_xxx"

# 按名称筛选指定工作表
curl -s "https://your-domain.com/api/open/v1/documents/42/sheet-data?sheet=员工信息" \
  -H "X-API-Key: ak_xxx"

# 按索引筛选（0 开始）
curl -s "https://your-domain.com/api/open/v1/documents/42/sheet-data?sheet=0" \
  -H "X-API-Key: ak_xxx"
```
