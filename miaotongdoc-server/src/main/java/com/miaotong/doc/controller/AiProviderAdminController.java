...
public class AiProviderAdminController {
    ...
    private Map<String, Object> toMap(AiProvider p) {
        Map<String, Object> m = new java.util.LinkedHashMap<>();
        m.put("id", p.getId());
        m.put("type", p.getType());
        m.put("name", p.getName());
        m.put("baseUrl", p.getBaseUrl());
        // v2.7.1：返回明文 API Key（前端"小眼睛"切换显隐）
        // 安全考虑：1) 仅 admin 可见（@PreAuthorize 拦截） 2) 必须从后端调（不暴露公网）
        m.put("apiKey", p.getApiKey() == null ? "" : configService.decryptKey(p.getApiKey()));
