...
var modelNameInputEl = document.getElementById('model-name-input');
var testConnectionBtnEl = document.getElementById('test-connection-btn');
...
function onModelInfo(info) {
	...
	if(info.model) {
		...
		modelNameInputEl.value = aiModel.id;
...
function testConnection() {
	var url = providerUrlInputEl.value.trim();
	var key = providerKeyInputEl.value.trim();
	var model = modelNameInputEl.value.trim();

	if (!url) {
		showTestResult(false, 'Please enter a URL');
		return;
	}
	if (!model) {
		showTestResult(false, 'Please enter a model name');
		return;
	}

	testConnectionBtnEl.disabled = true;
	testConnectionBtnEl.textContent = 'Testing...';
	...
	var proxyUrl = (window.AI && window.AI.serverSettings && window.AI.serverSettings.proxy) ? window.AI.serverSettings.proxy : null;

	var fetchUrl = proxyUrl || url + '/v1/chat/completions';
	var fetchOptions = {
		method: 'POST',
		headers: headers,
	...
	fetch(fetchUrl, fetchOptions)
		...
		.finally(function() {
			...
			testConnectionBtnEl.textContent = '测试连通性';
		});
}

function showTestResult(success, message) {
	var existing = document.getElementById('test-result-msg');
	if (existing) existing.remove();

	var el = document.createElement('div');
	el.id = 'test-result-msg';
	el.style.marginTop = '4px';
	el.style.fontSize = '12px';
	el.style.color = success ? '#4caf50' : '#f44336';
	el.textContent = message;
	testConnectionBtnEl.parentNode.appendChild(el);
