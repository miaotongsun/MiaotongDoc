...
- **缓存**：Redis 7
- **消息队列**：RabbitMQ 3
- **文档编辑**：MTOffice Document Server 9.3
- **对象存储**：MinIO（可选）
- **部署**：Docker Compose
...
│
├── scripts/                      # 运维脚本
│   ├── clean-editor-cache.sh     # MTOffice 缓存清理
...
└── data/                         # 运行时数据（.gitignore，不提交到 Git）
    ...
    ├── editor/                   # MTOffice 数据（配置、字体）
    ├── editor-cache/             # MTOffice 缓存
    └── logs/                     # 所有日志
        ├── server/               # 后端应用日志
        ├── nginx/                # Nginx 日志
        ├── postgres/             # PostgreSQL 日志
        ├── editor/               # MTOffice 编辑器1 日志
        ├── editor2/              # MTOffice 编辑器2 日志
        ├── editor3/              # MTOffice 编辑器3 日志
        ├── redis/                # Redis 日志
...
| `SECURE_LINK_SECRET` | MTOffice 安全链接密钥 | >= 16 位 |
...
| editor | miaotongdoc-editor | - | MTOffice 编辑器1 |
| editor2 | miaotongdoc-editor2 | - | MTOffice 编辑器2 |
| editor3 | miaotongdoc-editor3 | - | MTOffice 编辑器3 |
| minio | miaotongdoc-minio | 9000, 9001 | MinIO 对象存储 |
| cache-cleaner | miaotongdoc-cache-cleaner | - | MTOffice 缓存清理 |
...
RabbitMQ 用于 **MTOffice 多编辑器实例之间的协同编辑消息传递**。当多个用户同时编辑同一文档时，编辑器实例通过 RabbitMQ 交换光标位置、编辑操作等实时消息。
...
Dockerfile 对原始 MTOffice 镜像做了以下定制：
...
| 品牌替换 | 所有 "MTOffice" 文本替换为 "MiaotongDoc Editor" |
...
| `/socket.io/*` | 编辑器 | MTOffice Socket.IO 协同编辑 |
| `/ds-vpath/*` | 编辑器 | MTOffice 文档服务代理 |
