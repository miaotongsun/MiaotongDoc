 ...
 * Section 7(a) of the GNU AGPL version 3.
 *
...
function showRefreshResult(success, message) {
	var existing = document.getElementById('refresh-result-msg');
	if (existing) existing.remove();

	var el = document.createElement('div');
	el.id = 'refresh-result-msg';
	el.style.cssText = 'position:fixed;bottom:40px;left:50%;transform:translateX(-50%);padding:6px 16px;border-radius:4px;font-size:12px;z-index:9999;';
	el.style.background = success ? '#f0f9eb' : '#fef0f0';
	el.style.color = success ? '#67c23a' : '#f56c6c';
	el.style.border = success ? '1px solid #e1f3d8' : '1px solid #fbc4c4';
