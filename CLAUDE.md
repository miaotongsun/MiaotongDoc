...
| 6 | **UI 改动必跑 E2E** | 改 Vue 组件/页面交互后必须 `npm run e2e`,报告路径附在自检中 |
...
| 测试脚本 | `miaotongdoc-web/tests/` | E2E 测试|
| 统一入口 | `run.mjs` | `miaotongdoc-web/tests/` | 检查 Chromium + 自动装 + 跑测试 |
| 报告输出 | `*-e2e-report.md` | 同上(gitignore) | 本地生成,不入 git |
| 截图 | `tests/screenshots/*.png` | 同上(gitignore) | 本地生成,不入 git |
| 临时测试 | `tests/local/*.mjs` | `miaotongdoc-web/tests/local/`(gitignore) | 个人调试,不进 git |
...
| 阶段 | 关键产出 | G 门禁（只列关键问题）|
|---|---|---|
| ① 需求分析 | 3-5 句需求摘要 + 受影响文件清单 | 需求清晰无歧义?影响范围已穷举?无未澄清假设? |
| ② 计划设计 | `plans/YYYY-MM-DD-xxx.md` 计划文档 | 方案覆盖所有需求?风险有应对?验证标准可操作? |
| ③ 代码实现 | 代码 + 单元测试 | 分层规范?(Controller→Service→Repository)安全编码?(SQL/XSS/权限)null 处理? |
| ④ 代码审查 | 自审清单 + `/code-review` 报告 | 无 P0/P1 问题?所有审查发现已修复或接受? |
| ⑤ 功能验证 | API/DB/UI 三层验证 + E2E 报告 | 命令+输出?(禁"应该没问题")E2E?(UI 改动)回归无副作用? |
| ⑥ 文档归档 | CLAUDE.md / plans/ 已同步 | plans/README.md 看板已更新?提交信息引用 plans/ 路径? |
...
| 🟢 简单 | 单文件、≤10 行、纯 bugfix | 跳过计划，直接修 → 自检 → 验证 |
| 🟡 中等 | 2-5 文件、逻辑修改 | 口头/简短计划（TodoWrite）→ 实现 → 自检 → 验证 |
| 🔴 复杂 | 多文件、新功能、架构变更、数据库迁移 | **必须写** `plans/YYYY-MM-DD-xxx.md` |

**详细模板/纪律的落点**（不在此重复,避免双速查）:

- **计划文档模板 + ADR 模板** → `plans/README.md`（[§计划文档模板](plans/README.md#计划文档模板) / [§ADR 模板](plans/README.md#adr-模板)）
- **编码纪律 5 条**（单一职责/开闭/最少知识/防御编程/失败快速/DRY）→ §0 核心原则 + 日常 OOP 实践
- **安全编码 5 项**（SQL/XSS/权限/脱敏/校验）→ G3 自检 + OWASP 速查
- **提交规范 + 模板** → `plans/README.md`（[§提交规范](plans/README.md#提交规范)）
- **完整 E2E 工作流** → 顶部 [§🧪 E2E 自动化测试](#-e2e-自动化测试playwright) + 铁律 6 + G5 门禁
...
**详细模板与 frontmatter 规范** → [plans/README.md](plans/README.md)（计划文档模板、ADR 模板、提交规范、状态机）。
